...
/**
 * Generated by PHPUnit_SkeletonGenerator 1.2.1 on 2014-03-11 at 00:26:12.
 */
class AuthenticatorFilterTest extends \PHPUnit_Framework_TestCase
{
        ...
        {
                $this->filter = new AuthenticatorFilter((new AuthenticatorTree())->object);
        }
        ...
        public function testFind()
        {
                ...
                foreach ($this->filter->find('auth4') as $key => $obj) {
                        printf("(i) authenticator(...): %s -> %s\n", $key, print_r($obj, true));
                        $this->fail();
                }
        }
        ...
        public function testChain()
        {
                ...
                foreach ($this->filter->chain('chain1') as $chain) {
                        foreach ($chain as $key => $obj) {
                                ...
                                $this->assertTrue(in_array($key, $keys));
                                if ($key[0] == 'c') {
                                        $this->assertTrue($obj instanceof AuthenticatorChain);
                                } else {
                                        $this->assertTrue($obj instanceof HostnameAuthenticator);
                                }
                        }
        ...
        public function testAuthenticator()
        {
                ...
                foreach ($this->filter->authenticator('auth4') as $key => $obj) {
                        printf("(i) authenticator(...): %s -> %s\n", $key, print_r($obj, true));
                        $this->fail();
                }
        }
